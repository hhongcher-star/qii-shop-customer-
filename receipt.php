    ...
    <table>
      ...
      foreach ($order_data['items'] as $item) {
        ...
        if (!empty($item['variant_name'])) {
          echo " <span style='color:#C86A9B;'>（" . htmlspecialchars($item['variant_name']) . "）</span>";
        }
        echo "</td>
      ...
      // Apply coupon discount from session, per-order if available
...
function applyCoupon(){
    ...
    fetch("validate_coupon.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: "code=" + encodeURIComponent(code) + "&total=<?= $grand_total ?>"
    })
    .then(res => res.json())
    .then(data => {
        ...
        if (data.success) {
            // Save discount to session, then record usage in DB, finally reload
            fetch("save_coupon.php", {
                method: "POST",
                headers: { "Content-Type": "application/x-www-form-urlencoded" },
                body: "discount=" + encodeURIComponent(data.discount)
            })
            .then(() => {
                return fetch("use_coupon.php", {
                    method: "POST",
                    headers: { "Content-Type": "application/x-www-form-urlencoded" },
